...
!!! warning "`iconocards` has been obsolesced by Insiders"

    If you're using [Material for MkDocs Insiders](https://squidfunk.github.io/mkdocs-material/insiders/) 4.33.0 or
    later, the built-in social plugin has undergone a rewrite that renders `iconocards` no longer necessary. You can
    safely remove `iconocards` from `mkdocs.yml` and replace it with the built-in social plugin. You also no longer
    need to declare `iconoclast` first — the order no longer matters.

    If you're not using Insiders or if you're using an outdated version of Insiders, you must continue to use
    `iconocards` as described below.

    `iconocards` will be deprecated when the rewritten social plugin makes its way into the public edition of
    Material for MkDocs. It will be removed in the next major release of Iconoclast after that.
